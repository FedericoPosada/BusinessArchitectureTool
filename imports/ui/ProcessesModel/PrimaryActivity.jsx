...
export default class PrimaryActivity extends React.Component {
    constructor(props) {
        super(props)
        this.state={
            isInEditMode: false
    ...
    updateActivity(){
        ...
        processesContainer.update(query, {$set:{name:newname}}, (err, done) => {
            if (err)
                Materialize.toast("Ha ocurrido un error al actualizar la actividad",3000);
        });
    }
    deleteActivity(){
        var id = this.props._id;
        processesContainer.remove({_id:id});
    }
    changeEditMode(){
    ...
    renderEdit(){
        return(
            <>
                ...
                <td style={{width:"5%"}}><a onClick={this.deleteActivity.bind(this)} className="waves-effect waves-light btn red" style={{marginRight:5,marginBottom:5}}><i
