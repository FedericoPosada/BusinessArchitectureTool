...
export default class OrgChartCreator extends React.Component{
    ...
    componentWillMount() {
        Tracker.autorun(()=> {
            let query=OrgCharts.collection.findOne({userId:Meteor.userId()});
    ...
    uploadIt(e) {
        ...
        if (e.currentTarget.files && e.currentTarget.files[0]) {
            ...
            if (file) {
                let uploadInstance = OrgCharts.insert({
                    file: file,
                    streams: 'dynamic',
                    chunkSize: 'dynamic',
    ...
    removeFile(){
        Meteor.call('RemoveOrgChart', this.state.imageid, function (err, res) {
            if (err)
    ...
    render() {
        let query=OrgCharts.findOne({_id:this.state.imageid});
        let cursor;
        if(typeof query !== "undefined")
            cursor=query.link();
        else
