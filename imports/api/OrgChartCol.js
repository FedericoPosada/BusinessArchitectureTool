...
const OrgCharts = new FilesCollection({
    storagePath: 'C:\\orgcharts',
    downloadRoute: '/files/orgcharts',
    collectionName: 'orgcharts',
    permissions: 0o755,
    ...
    downloadCallback(fileObj) {
        if (this.params.query.download == 'true') {
            ...
            OrgCharts.update(fileObj._id, {$inc: {'meta.downloads': 1}});
        }
...
if (Meteor.isClient) {
    Meteor.subscribe('OrgCharts');
}

if (Meteor.isServer) {
    // This code only runs on the server
    Meteor.publish('OrgCharts', function () {
        return OrgCharts.find().cursor;
    });
}
