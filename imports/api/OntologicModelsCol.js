...
const OntologicModels = new FilesCollection({
    storagePath: 'C:\\ontologicmodels',
    downloadRoute: '/files/ontologicmodels',
    collectionName: 'ontologicmodels',
    permissions: 0o755,
    ...
    downloadCallback(fileObj) {
        if (this.params.query.download == 'true') {
            ...
            OntologicModels.update(fileObj._id, {$inc: {'meta.downloads': 1}});
        }
...
if (Meteor.isClient) {
    Meteor.subscribe('ontologicmodels');
}

if (Meteor.isServer) {
    // This code only runs on the server
    Meteor.publish('ontologicmodels', function () {
        return OntologicModels.find().cursor;
    });
}

// Export FilesCollection instance, so it can be imported in other files
export default OntologicModels;